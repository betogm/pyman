...
-   `shared` (object): A special object for sharing variables and functions across different scripts within the same collection run. This is particularly useful for `collection-pre-script.py` to set up global data or utility functions that can be accessed by individual request pre/post-scripts.
    ...
    **Example: Sharing Variables and Functions**
    ...
    # 1. Define a shared global variable
    # Any subsequent script can read or modify this value.
    shared.id_sessao_global = pm.random_uuid()
    log.info(f"Global Session ID defined: {shared.id_sessao_global}")

    # 2. Define a shared global function
    ...
    # 3. Attach the function to the 'shared' object
    # This makes 'shared.get_auth_token' globally accessible.
    shared.get_auth_token = get_auth_token

    # 4. You can also define environment variables (this already worked)
...
try:
    log.info(f"POS script: Global Session ID from shared: {shared.id_sessao_global}")

    # Using a shared function
    if response.status_code == 200:
        log.info("POS script: Request OK!")
        
        # Example of using the shared function defined in collection-pre-script.py
        new_token = shared.get_auth_token("user_from_pos", "pass_from_pos")
        log.info(f"New token generated by shared function: {new_token}")
        log.info(f"Last generated token from shared scope: {shared.ultimo_token_gerado}")
