...
1. **`pyman/pyman.py`**:
   ...
   - **Key Options**:
     ...
     - `--no-ai-report`: Disables AI diagnosis report (Markdown) generation.
...
5. **`pyman/log_reporter.py`**:
   - **Role**: Parses execution log files and/or reads JSON reports, converting them into fully responsive HTML reports and token-efficient AI diagnosis reports (Markdown format) inside the `reports/` folder.
...
### 5. Nested Chained Requests in Log Reporter (Fixed)
- **Context**: When a request triggered pre-requests recursively, the flat parser in `log_reporter.py` could not distinguish the hierarchical log output structure, causing subsequent requests to be wrongly merged or report incorrect status codes (e.g. `None N/A` for `get-data.yaml`).
- **Code Status**: 
  - Aligned JSON report and log file naming timestamps by extracting the logger FileHandler's file path.
  - Implemented `get_report_data` in `log_reporter.py` to automatically load report execution data directly from the structured JSON file when it is available, falling back to raw log parsing only when JSON is absent. This ensures 100% correct status, header, and body reporting for chained requests.
